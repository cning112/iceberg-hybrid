...
/**
 * Routes read requests to the optimal region for low-latency access. Implements intelligent routing
 * based on data availability and region proximity. Provides fallback mechanisms when data is not
 * available locally.
 */
@Slf4j
...
public class ReadRouter {
  ...
  public Optional<TableMetadata> getLatestMetadata(TableId tableId, Region preferredRegion) {
    log.debug(
        "Getting latest metadata for table {} preferring region {}", tableId, preferredRegion);
   ...
   */
  public Optional<TableMetadata> getMetadata(
      TableId tableId, CommitId commitId, Region preferredRegion) {
    log.debug(
        "Getting metadata for table {} commit {} preferring region {}",
        tableId,
        commitId,
        preferredRegion);
  ...
  public Optional<Region> getBestReadRegion(TableId tableId, Region preferredRegion) {
    ...
    for (Region region : availableRegions) {
      if (registryPort.isRegionActive(region) && isRegionDataComplete(tableId, region)) {
    ...
    log.warn(
        "Using last resort region {} for table {} (data may be incomplete)",
        fallbackRegion,
        tableId);
  ...
  public Optional<StorageLocation> getStorageLocation(TableId tableId, Region region) {
    try {
      ...
      log.debug("Found storage location for table {} in region {}: {}", tableId, region, storage);
      return Optional.of(storage);

    } catch (Exception e) {
      log.error("Failed to get storage location for table {} in region {}", tableId, region, e);
      return Optional.empty();
    }
  }
  ...
  /**
   * Gets the data file paths for a table in a specific region. Used by query engines to locate the
   * actual data files.
   *
  ...
  public List<String> getDataFiles(TableId tableId, Region region, Optional<CommitId> commitId) {
    try {
      ...
      Optional<TableMetadata> metadataOpt =
          commitId.isPresent()
  ...
  /**
   * Checks if a region has complete data for a table. This is a simplified check - in practice,
   * would verify file existence and completeness.
   */
  private boolean isRegionDataComplete(TableId tableId, Region region) {
    try {
      ...
      log.error("Failed to check data completeness for table {} in region {}", tableId, region, e);
      return false;
    }
  }

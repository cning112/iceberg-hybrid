...
/**
 * Coordinates write operations across the geo-distributed system. Implements the distributed commit
 * protocol ensuring consistency across regions. Handles the complete write workflow from commit
 * request to global replication.
 */
@Slf4j
...
public class WriteCoordinator {
        ...
        .whenComplete(
            (commitId, throwable) -> {
              if (throwable != null) {
                ...
                log.info(
                    "Write coordination completed successfully for {} with commit {}",
                    request,
                    commitId);
              }
            });
    ...
    if (result.isSuccess()) {
      ...
      return CompletableFuture.failedFuture(
          new IllegalStateException(
              "Commit approval failed: " + result.getErrorMessage().orElse("Unknown error")));
    }
  }
            ...
            targetRegion -> {
              try {
                ...
                if (!request.getNewDataFiles().isEmpty()) {
                  SyncEvent dataEvent =
                      syncPort.createDataSyncEvent(
                          metadata, request.getNewDataFiles(), targetRegion);
                  syncPort.publishSyncEvent(dataEvent);
                }

                log.debug(
                    "Created sync events for region {} and commit {}", targetRegion, commitId);
              } catch (Exception e) {
                log.error(
                    "Failed to create sync events for region {} and commit {}",
                    targetRegion,
                    commitId,
                    e);
              }
  ...
  private String generateTableDataPath(TableId tableId) {
    return String.format("/data/%s/%s/%s", tableId.namespace(), tableId.name(), UUID.randomUUID());
  }
}

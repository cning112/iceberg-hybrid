...
/**
 * End-to-end integration test for the hybrid geo-distributed Iceberg system. Tests the complete
 * flow from write coordination through regional synchronization to read routing across multiple
 * regions.
 *
 * <p>This test uses all in-memory adapters to simulate a complete multi-region deployment and
 * verifies that writes are properly coordinated, synchronized, and available for reads across
 * regions.
 */
@Slf4j
public class HybridSystemEndToEndTest {
  ...
  private static final String TEST_SCHEMA =
      "{\"type\":\"struct\",\"fields\":[{\"id\":1,\"name\":\"id\",\"type\":\"long\"}]}";
  ...
  /**
   * Sets up the complete hybrid system with three regions for testing. Configures all adapters and
  ...
  void setupHybridSystem() {
    ...
    writeCoordinator =
        new WriteCoordinator(catalogAdapter, commitGateAdapter, syncAdapter, registryAdapter);

    syncOrchestrator =
        new SyncOrchestrator(syncAdapter, storageAdapter, catalogAdapter, registryAdapter);

    readRouter = new ReadRouter(catalogAdapter, storageAdapter, registryAdapter);

    log.info(
        "Hybrid system setup completed with {} regions", registryAdapter.getAllRegions().size());
  }

  /**
   * Tests the complete write-to-read flow across multiple regions.
   *
   * <p>Flow: 1. Create table in the source region 2. Execute a write operation with commit
   * coordination 3. Verify metadata exists in catalog 4. Test read routing functionality 5. Verify
  ...
  void testCompleteWriteToReadFlow() throws Exception {
    ...
    assertEquals(commitId, catalogMetadata.get().getCommitId(), "Commit IDs should match");
    ...
    assertEquals(
        "s3://iceberg-us-east", storageLocation.get().uri(), "Should match configured storage");
  ...
  /**
   * Tests write failure scenarios and rollback behavior. Simulates commit gate rejection and
   * verifies system handles it gracefully.
   */
  @Test
  void testWriteFailureAndRollback() throws Exception {
    ...
    commitGateAdapter.setRequiredApprovalRegions(failTable, List.of(US_EAST, EU_WEST, fakeRegion));

    // First create the table
    writeCoordinator.createTable(failTable, TEST_SCHEMA, US_EAST);

    // Create a commit request that should fail approval
    CommitRequest failRequest =
        new CommitRequest(
            failTable,
            US_EAST,
            Instant.now(),
  ...
  void testSystemMonitoring() {
    ...
    assertTrue(
        readRouter.tableExists(TEST_TABLE) || !readRouter.tableExists(TEST_TABLE),

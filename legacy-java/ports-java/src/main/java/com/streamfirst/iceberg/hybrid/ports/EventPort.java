...
/**
 * Port for asynchronous event messaging infrastructure. Provides publish/subscribe capabilities for
 * inter-service communication.
 */
public interface EventPort {

  /**
   * Publishes an event synchronously to a topic. Blocks until the event is delivered to the message
   * broker.
   *
  ...
  /**
   * Publishes an event asynchronously to a topic. Returns immediately without waiting for delivery
   * confirmation.
   *
   * @param topic the topic to publish to
   * @param event the event object to publish
  ...
  /**
   * Subscribes to a topic with type-safe event handling. Only events of the specified type will be
  ...
  /**
   * Unsubscribes subscriptions matching the specified criteria. Provides flexible subscription
   * management for complex event scenarios.
   *
  ...
  default int unsubscribeFromTopic(String topic) {
    return unsubscribeMatching(
        subscriptionId -> getSubscriptionTopic(subscriptionId).equals(topic));
  }
  ...
  /**
   * Closes the event port and releases resources. Unsubscribes from all topics and closes
   * connections.
   */
  void close();
}

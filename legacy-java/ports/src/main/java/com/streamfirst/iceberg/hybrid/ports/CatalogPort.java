...
/**
 * Port for global transactional catalog operations. Provides access to the single source of truth
 * for table metadata across all regions.
 */
public interface CatalogPort {

  /**
   * Commits new metadata to the global catalog. This is an atomic operation that ensures
  ...
  /**
   * Gets commits for a table matching specific criteria. Used for flexible synchronization queries
   * between regions.
   *
   * @param tableId the table identifier
  ...
  /**
   * Gets all commits for a table since a specific commit ID. Used for incremental synchronization
   * between regions.
   *
   * @param tableId the table identifier
  ...
  default List<TableMetadata> getCommitsSince(TableId tableId, CommitId sinceCommitId) {
    return getCommits(
        tableId, metadata -> metadata.getCommitId().value().compareTo(sinceCommitId.value()) > 0);
  }

  /**
   * Lists tables matching specific criteria. Allows flexible filtering of tables across namespaces.
   *
  ...
  /**
   * Drops a table from the catalog. This removes all metadata history for the table.
   *
   * @param tableId the table identifier

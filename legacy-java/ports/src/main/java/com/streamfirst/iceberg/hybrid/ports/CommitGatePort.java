...
/**
 * Port for regional commit gate coordination. Ensures write consistency by requiring approval from
 * all regions before committing. Implements the distributed consensus mechanism for geo-distributed
 * writes.
 */
public interface CommitGatePort {

  /**
   * Requests commit approval from all required regions. This initiates the distributed commit
   * protocol.
   *
  ...
  /**
   * Records rejection for a commit from a specific region. This will cause the entire commit to be
   * rejected.
   *
  ...
  /**
   * Gets commit approvals matching the specified criteria. Provides flexible querying for complex
   * commit gate scenarios.
   *
  ...
  /**
   * Gets all pending commit requests for a specific region. Used by regional commit gate workers to
   * process their approval queue.
   *
  ...
  /**
   * Gets commit requests matching the specified criteria. Provides flexible querying for complex
   * commit gate scenarios.
   *
  ...
  default List<CommitRequest> getCommitRequests(Predicate<CommitRequest> predicate) {
    return getCommitApprovals(approval -> predicate.test(approval.request())).stream()
        .map(CommitApproval::request)
        .distinct()
        .toList();
  }
  ...
  /**
   * Releases the commit lock, allowing other commits to proceed. Called after successful commit or
   * when cleaning up failed commits.
   *
  ...
  /**
   * Gets the list of regions that must approve commits for a table. This depends on the table's

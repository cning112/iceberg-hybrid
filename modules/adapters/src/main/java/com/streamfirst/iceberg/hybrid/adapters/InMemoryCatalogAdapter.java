 ...
 * Provides a simple, thread-safe catalog that stores all metadata in memory.
...
public class InMemoryCatalogAdapter implements CatalogPort {
    ...
    public CommitId commitMetadata(CommitRequest request) {
        ...
        // Register namespace
        namespaces.computeIfAbsent(request.getTableId().namespace(), k -> new HashSet<>())
                  .add(request.getTableId());
        
        log.info("Committed metadata for table {} with commit ID {}", 
                 request.getTableId(), commitId);
        return commitId;
    }
    ...
    public Optional<TableMetadata> getLatestMetadata(TableId tableId) {
        ...
        log.debug("Retrieved latest metadata for table {}: commit {}", 
                 tableId, latest.getCommitId());
    ...
    public void createTable(TableId tableId, String schema, Region sourceRegion) {
        ...
        TableMetadata initialMetadata = new TableMetadata(
            tableId,
            initialCommit,
            sourceRegion,
            Instant.now(),
    ...
    public int getTotalCommitCount() {
        return tableHistory.values().stream()
                .mapToInt(List::size)
                .sum();
    }
}
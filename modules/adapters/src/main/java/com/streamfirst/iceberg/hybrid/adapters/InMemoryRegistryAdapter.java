...
/**
 * In-memory implementation of RegistryPort for testing and development. Provides simple registry
 * functionality using in-memory maps. Data is lost when the application stops - not suitable for
 * production use.
 */
@Slf4j
public class InMemoryRegistryAdapter implements RegistryPort {
  ...
  private final Map<Region, Map<String, StorageLocation>> storageLocationsByRegion =
  ...
  public void registerStorageLocation(StorageLocation location) {
    ...
    if (regionStorageMap != null && regionStorageMap.containsKey(type)) {
      ...
      if (!existing.equals(location)) {
        throw new IllegalArgumentException(
            "Storage location conflict for region "
                + region
                + " type "
                + type
                + ": existing="
    ...
    log.info("Registered storage location for region {} type {}: {}", region, type, location.uri());
  }
  ...
  public void updateStorageLocation(StorageLocation location) {
    ...
    log.info("Updated storage location for region {} type {}: {}", region, type, location.uri());
  }
    ...
    while (regionIterator.hasNext()) {
      ...
      while (typeIterator.hasNext()) {
        ...
        if (predicate.test(location)) {
          ...
          removedCount++;
          log.info(
              "Removed storage location for region {} type {}: {}",
              region,
              typeEntry.getKey(),
              location.uri());
        }
  ...
  public Optional<StorageLocation> getStorageLocation(Region region, String type) {
    ...
    if (location != null) {
      log.debug("Found storage location for region {} type {}: {}", region, type, location.uri());
    } else {
  ...
  public List<StorageLocation> getStorageLocationsByType(String type) {
    List<StorageLocation> locations =
        storageLocationsByRegion.values().stream()
  ...
  public void registerTableLocation(TableId tableId, Region region, String dataPath) {
    log.debug(
        "Registering table location: table={}, region={}, path={}", tableId, region, dataPath);

    tableLocations.computeIfAbsent(tableId, k -> new ConcurrentHashMap<>()).put(region, dataPath);
  ...
  public Map<String, Integer> getRegistryStats() {
    ...
    stats.put(
        "active_regions", (int) regionStatus.values().stream().mapToInt(b -> b ? 1 : 0).sum());
    stats.put(
        "storage_locations", storageLocationsByRegion.values().stream().mapToInt(Map::size).sum());
    stats.put("tables", tableLocations.size());
    stats.put("table_locations", tableLocations.values().stream().mapToInt(Map::size).sum());

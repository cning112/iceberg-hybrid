...
import com.streamfirst.iceberg.hybrid.ports.{RegistryPort, RegionStatus}
import zio.*
import zio.logging.*
...
final case class InMemoryRegistryAdapter() extends RegistryPort:
  ...
  override def getTableDataPath(tableId: TableId, region: Region): IO[StorageError, Option[String]] =
    ZIO.succeed {
      tableLocations.get((tableId, region))
    }

  override def registerTableLocation(tableId: TableId, region: Region, dataPath: String): IO[StorageError, Unit] =
    ZIO.succeed {
      tableLocations.put((tableId, region), dataPath)
      ()
    }.tap(_ => ZIO.logDebug(s"Registered table location: $tableId in region ${region.id} at $dataPath"))
  ...
  override def registerRegion(region: Region, storageLocation: StorageLocation): IO[ConfigurationError, Unit] =
    ZIO.succeed {
  ...
  override def updateRegionStatus(region: Region, status: RegionStatus): IO[ConfigurationError, Unit] =
    ZIO.succeed {
...
object InMemoryRegistryAdapter:
  ...
  def withSampleData: ZLayer[Any, Nothing, RegistryPort] =
    ZLayer.fromZIO {
      ...
      val usEast1Storage = StorageLocation.s3(Region.UsEast1, "iceberg-data-us-east-1", Some("tables"))
      val euWest1Storage = StorageLocation.s3(Region.EuWest1, "iceberg-data-eu-west-1", Some("tables"))

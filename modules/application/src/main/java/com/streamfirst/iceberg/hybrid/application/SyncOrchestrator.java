...
/**
 * Orchestrates synchronization operations between regions.
 * Processes sync events and coordinates data/metadata replication.
 * Handles both metadata synchronization and data file copying.
 */
@Slf4j
...
public class SyncOrchestrator {
    ...
    /**
     * Processes all pending sync events for a specific region.
     * This method is typically called by a scheduled worker process.
     * 
    ...
    public int processPendingEvents(Region region) {
        ...
        log.info("Processed {} out of {} pending events for region {}", 
                 successCount, pendingEvents.size(), region);
        return successCount;
    }
    ...
    private boolean processSyncEvent(SyncEvent event) {
        ...
        try {
            boolean success = switch (event.getType()) {
    ...
    private boolean processMetadataSync(SyncEvent event) {
        log.debug("Processing metadata sync for table {} to region {}", 
                 event.getTableId(), event.getTargetRegion());
        ...
        try {
            ...
            if (metadataOpt.isEmpty()) {
                log.warn("Metadata not found for table {} commit {}", 
                        event.getTableId(), event.getCommitId());
                return false;
            }
            ...
            if (existingPath.isEmpty()) {
                ...
                log.debug("Registered table location for {} in region {}", 
                         event.getTableId(), event.getTargetRegion());
            }
    ...
    private boolean processDataSync(SyncEvent event) {
        log.debug("Processing data sync for table {} from {} to {}", 
                 event.getTableId(), event.getSourceRegion(), event.getTargetRegion());
        ...
        try {
            ...
            String targetBasePath = registryPort.getTableDataPath(event.getTableId(), event.getTargetRegion())
            ...
            log.info("Copied {} files for table {} from {} to {}", 
                    copiedFiles, event.getTableId(), event.getSourceRegion(), event.getTargetRegion());
            return true;

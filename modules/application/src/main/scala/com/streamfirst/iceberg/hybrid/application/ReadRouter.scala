...
/**
 * Routes read operations to the optimal region based on data locality and availability.
 * Implements intelligent routing for geo-distributed read queries.
 */
final case class ReadRouter(
  ...
  def routeRead(
    ...
    for
      ...
      availableRegions <- registryPort.getTableRegions(tableId)
      ...
      storageLocation <- storagePort.getStorageLocation(bestRegion)
        .mapError(identity)
  ...
  private def chooseBestRegion(
    ...
    preferredRegion match
      ...
      case _ =>
        ...
        ZIO.fromOption(availableRegions.headOption)

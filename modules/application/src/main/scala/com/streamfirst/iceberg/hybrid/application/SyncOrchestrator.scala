...
final case class SyncOrchestrator(
  ...
  def processPendingEvents(region: Region): IO[SyncError, Int] =
    for
      ...
      _ <- ZIO.logInfo(s"Processed $successCount out of ${pendingEvents.size} pending events for region ${region.id}")
    yield successCount
  ...
  private def processSyncEvent(event: SyncEvent): IO[SyncError, Boolean] =
    for
      ...
      _ <- if success then
        ZIO.logDebug(s"Successfully processed sync event ${event.eventId}")
      else
        ZIO.logWarning(s"Failed to process sync event ${event.eventId}")
    yield success
  ...
  private def processMetadataSync(event: SyncEvent): IO[SyncError, Boolean] =
    for
      _ <- ZIO.logDebug(s"Processing metadata sync for table ${event.tableId} to region ${event.targetRegion.id}")
      ...
      _ <- existingPath match
        ...
        case None =>
          ...
          registryPort.registerTableLocation(event.tableId, event.targetRegion, dataPath)
            .mapError(storageErrorToSyncError) *>
          ZIO.logDebug(s"Registered table location for ${event.tableId} in region ${event.targetRegion.id}")
    yield true
  ...
  private def processDataSync(event: SyncEvent): IO[SyncError, Boolean] =
    for
      _ <- ZIO.logDebug(s"Processing data sync for table ${event.tableId} from ${event.sourceRegion.id} to ${event.targetRegion.id}")
      ...
      _ <- ZIO.logInfo(s"Copied $copiedFiles files for table ${event.tableId} from ${event.sourceRegion.id} to ${event.targetRegion.id}")
    yield true
  ...
  def retryFailedEvents(region: Region): IO[SyncError, Int] =
    for
      ...
      retriedCount <- ZIO.foldLeft(failedEvents)(0) { (count, event) =>
        syncPort.retryFailedEvent(event.eventId)
          .as(count + 1)

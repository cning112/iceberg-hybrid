...
/**
 * Port for global storage and region registry. Maintains the mapping of storage locations, table
 * locations, and region configurations. Acts as the service discovery mechanism for the
...
public interface RegistryPort {
  ...
  /**
   * Removes storage locations matching the given predicate. This provides maximum flexibility for
  ...
  /**
   * Removes a specific storage location by region and type. Convenience method for the most common
   * removal pattern.
   *
  ...
  default boolean removeStorageLocation(Region region, String type) {
    return removeStorageLocations(
            location -> location.region().equals(region) && location.type().equals(type))
        > 0;
  }
  ...
  /**
   * Updates the active status of a region. Inactive regions don't participate in commit consensus.
   *
   * @param region the region

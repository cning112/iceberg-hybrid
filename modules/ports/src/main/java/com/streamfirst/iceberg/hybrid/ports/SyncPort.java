 ...
 * and data between geographic regions.
 */
public interface SyncPort {
  ...
  default List<SyncEvent> getPendingEvents(Region targetRegion) {
    return getSyncEvents(
        event ->
            event.getTargetRegion().equals(targetRegion)
  ...
  default List<SyncEvent> getEventHistory(TableId tableId, Region region) {
    return getSyncEvents(
        event -> event.getTableId().equals(tableId) && event.getTargetRegion().equals(region));
  }
  ...
  default List<SyncEvent> getFailedEvents(Region region) {
    return getSyncEvents(
        event ->
            event.getTargetRegion().equals(region) && event.getStatus() == SyncEvent.Status.FAILED);
  }

...
/**
 * Port for the global commit gate that ensures write consistency across regions.
 * Coordinates distributed consensus before allowing commits to proceed.
 */
trait CommitGatePort:
  /**
   * Requests approval for a commit from the global commit gate.
   * This ensures no conflicting writes are happening across regions.
   */
  def requestCommitApproval(request: CommitRequest): IO[SyncError, CommitApproval]

  /**
   * Notifies the commit gate that a commit has been completed in a region.
   * Used for cleanup and releasing locks.
   */
  def notifyCommitCompleted(commitId: CommitId, region: Region): IO[SyncError, Unit]

  /**
   * Notifies the commit gate that a commit has failed in a region.
   * Triggers rollback procedures across other regions if needed.
   */
...
object CommitGatePort:
   ...
   */
  def requestCommitApproval(request: CommitRequest): ZIO[CommitGatePort, SyncError, CommitApproval] =
    ZIO.serviceWithZIO[CommitGatePort](_.requestCommitApproval(request))

  def notifyCommitCompleted(commitId: CommitId, region: Region): ZIO[CommitGatePort, SyncError, Unit] =
    ZIO.serviceWithZIO[CommitGatePort](_.notifyCommitCompleted(commitId, region))

  def notifyCommitFailed(commitId: CommitId, region: Region, error: String): ZIO[CommitGatePort, SyncError, Unit] =

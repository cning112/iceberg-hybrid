...
import com.streamfirst.iceberg.hybrid.domain.DomainError.{StorageError, ConfigurationError}
import zio.*

/**
 * Port for managing the global registry of storage locations and regional configurations.
 * Tracks where table data is stored across different regions.
 */
trait RegistryPort:
  /**
...
object RegistryPort:
   ...
   */
  def getTableDataPath(tableId: TableId, region: Region): ZIO[RegistryPort, StorageError, Option[String]] =
    ZIO.serviceWithZIO[RegistryPort](_.getTableDataPath(tableId, region))

  def registerTableLocation(tableId: TableId, region: Region, dataPath: String): ZIO[RegistryPort, StorageError, Unit] =

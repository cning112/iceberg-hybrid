...
trait StoragePort:
  ...
  def writeFile(
    ...
    path: StoragePath,
    data: Array[Byte]): IO[StorageError, Unit]
  ...
  def writeFileStream(
    location: StorageLocation,
    path: StoragePath,
    data: ZStream[Any, Throwable, Byte]): IO[StorageError, Unit]
...
object StoragePort:
  ...
  def writeFile(
    location: StorageLocation,
    path: StoragePath,
    data: Array[Byte]): ZIO[StoragePort, StorageError, Unit] =
    ZIO.serviceWithZIO[StoragePort](_.writeFile(location, path, data))

  def readFile(
    location: StorageLocation,
    path: StoragePath): ZIO[StoragePort, StorageError, Array[Byte]] =
    ZIO.serviceWithZIO[StoragePort](_.readFile(location, path))

  def fileExists(
    location: StorageLocation,
    path: StoragePath): ZIO[StoragePort, StorageError, Boolean] =
